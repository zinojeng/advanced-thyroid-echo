...
/* --- 來源標示 ------------------------------------------------------------
 * 醫學教育資源不只在 YouTube：學會、期刊、醫院教學站與病例圖譜各有存取條件。
 * 使用者點進去之前就該知道：要不要註冊、能不能嵌入、授權是什麼、最後查核是哪天。 */

const SRC_UI = () => CFG.ui?.source || {};

export function sourceBadges(v) {
  if (!v || !v.url) return "";
  const ui = SRC_UI();
  const out = [];
  if (v.tier) {
    const t = (ui.tiers || {})[v.tier];
    if (t) out.push(`<span class="Label Label--${esc(t.tone || "neutral")}" title="${esc(t.title || "")}">${esc(t.label)}</span>`);
  }
  if (v.provider && v.provider !== "youtube") {
    const p = (ui.providers || {})[v.provider];
    if (p) out.push(`<span class="Label Label--neutral">${esc(p)}</span>`);
  }
  if (v.access && v.access !== "open") {
    const a = (ui.access || {})[v.access];
    if (a) out.push(`<span class="Label Label--attention" title="${esc(ui.accessNote || "")}">${esc(a)}</span>`);
  }
  if (v.source_type) {
    const st = (ui.sourceTypes || {})[v.source_type];
    if (st) out.push(`<span class="Label Label--neutral">${esc(st)}</span>`);
  }
  if (v.coi) out.push(`<span class="Label Label--danger" title="${esc(v.coi)}">${esc(ui.coiLabel || "利益關係")}</span>`);
  return out.join("");
}

export function sourceFootnote(v) {
  if (!v || !v.url) return "";
  const ui = SRC_UI();
  const bits = [];
  if (v.license) bits.push(`${esc(ui.licenseLabel || "授權")}：${esc(v.license)}`);
  if (v.last_verified) bits.push(`${esc(ui.verifiedLabel || "最後查核")}：${esc(v.last_verified)}`);
  return bits.length ? `<span class="VideoCard__src">${bits.join(" · ")}</span>` : "";
}
    ...
    return `
      <div class="VideoCard VideoCard--missing">
        ...
        <span class="VideoCard__main">
          <span class="VideoCard__title">${esc(CFG.ui?.missingTitle || "尚未找到合格資源")}</span>
          <span class="VideoCard__meta">${esc(v?.note || CFG.ui?.missingHint || "")}</span>
        </span>
        ${playBtn(false, true)}
      </div>`;
  }
  ...
  return `
    <a class="VideoCard" href="${esc(v.url)}" target="_blank" rel="noopener">
      ...
      <span class="VideoCard__main">
        ...
        ${sourceBadges(v) ? `<span class="VideoCard__badges">${sourceBadges(v)}</span>` : ""}
...
function drill(d) {
  const inner = `
    ...
    <span class="Drill__main">
      ...
      ${sourceBadges(d) ? `<span class="Drill__badges">${sourceBadges(d)}</span>` : ""}
