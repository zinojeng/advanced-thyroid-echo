...
import sources as SRC  # noqa: E402  來源判定（provider 推斷、URL 合法性）
...
def _age_days(iso: str | None) -> int | None:
    """last_verified 距今幾天。格式不對回 None。"""
    if not iso or not SRC.ISO_DATE.match(str(iso)):
        return None
    try:
...
def audit_videos(cfg: dict, units: list[dict], opts: dict, rep: Report) -> None:
    ...
    # metaCoverage 只對 YouTube 有意義：其他來源本來就沒有可程式化查詢的中繼資料
    yt_filled = filled - len(external) - len(malformed)
    coverage = hits / yt_filled if yt_filled else 1
    line = (
        f"{slots} 個影片欄位 · 去重 {len(seen)} 支 · "
        f"YouTube 中繼資料命中 {hits}/{yt_filled}（{coverage:.1%}）· "
        f"非 YouTube 來源 {len(external)} 個"
    )
    if yt_filled and coverage < opts["metaCoverage"]:
    ...
    if thin_meta:
        rep.err(
            sec,
            f"{len(thin_meta)} 個來源的中繼資料不完整"
            "（非 YouTube 來源必須標示 source_type／access／license／last_verified）",
            thin_meta,
        )
    if outdated:
        rep.warn(
