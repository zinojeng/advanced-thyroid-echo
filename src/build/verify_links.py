...
def main() -> int:
    ...
    # 非 YouTube 來源沒有 oEmbed，交給 verify_external.py
    urls = sorted({v["url"] for *_, v in nodes if v.get("url") and SRC.is_youtube(v)})
    external = len({v["url"] for *_, v in nodes if v.get("url")}) - len(urls)
    print(
        f"檢查 {len(urls)} 個 YouTube 連結（涵蓋 {len(nodes)} 個影片欄位）"
        f"；另有 {external} 個外部來源交給 make verify-external\n"
    )
    if not urls:
        return 0
