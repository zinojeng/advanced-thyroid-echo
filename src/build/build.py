...
def main() -> int:
    ...
    for _, _, _, src in CHAPTERS:
        if src not in source_blobs:
            source_blobs[src] = load_json(DATA / f"{src}.json")

    for code, title, ic, src in CHAPTERS:
        blob = source_blobs.get(src)
        if not blob:
        ...
        for u in units:
            ...
            for v in vids:
                ...
                if not url:
                    ...
                    continue
                if why := sources.url_problem(v):
                    bad_urls.append(f"{u['id']}: {why}")
                # 前端要靠 provider 決定嵌入或外連，沒寫就從網址推
                v["provider"] = sources.infer_provider(v)
                providers[v["provider"]] += 1
                v["embeddable"] = sources.embeddable(v)
                seen_urls[url] += 1
    ...
    course = {
        ...
        "meta": {
            ...
            # 非 YouTube 來源（學會、期刊、醫院、病例圖譜）佔多少
            "external_slots": sum(n for p, n in providers.items() if p != "youtube"),
